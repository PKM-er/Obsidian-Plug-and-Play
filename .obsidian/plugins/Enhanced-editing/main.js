...
class MyPlugin extends obsidian.Plugin {
    ...
    转换背景颜色() {
        ...
        let _html0 = /\<mark style=[\"'][^\<\>]+:[0-9a-zA-Z#]+[\"'][^\<\>]*\>[^\<\>]+\<\/mark\>/g;
        let _html1 = /^\<mark style=[\"'][^\<\>]+:[0-9a-zA-Z#]+[\"'][^\<\>]*\>([^\<\>]+)\<\/mark\>$/;
        let _html2 = '\<mark style=\"background\:'+this.settings.bColor+'\"\>$1\<\/mark\>';
        let _html3 = /\<mark style=[^\<]*$|^[^\>]*mark\>/g;	//是否只包含一侧的<>
        if (_html3.test(所选文本)){
            return; //new obsidian.Notice("不能转换颜色！");
        }else if (_html0.test(所选文本)){
            if(_html1.test(所选文本)){
                ...
                所选文本 = 所选文本.replace(/\<mark style=[\"'][^\<\>]+:[0-9a-zA-Z#]+[\"'][^\<\>]*\>|\<\/mark\>/g,"");
