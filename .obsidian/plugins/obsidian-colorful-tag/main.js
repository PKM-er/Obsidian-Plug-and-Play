...
var DEFAULT_SETTINGS = {
  ...
  defaultStyle: /* @__PURE__ */ new Map([["radius", "4px"], ["prefix", ""], ["suffix", ""], ["background-color", "#fff"], ["text-color", "#000"], ["text-size", "12px"], ["border", "none"], ["font-weight", "900"]]),
  defaultGlobal: /* @__PURE__ */ new Map([["radius", false], ["prefix", false], ["suffix", false], ["background-color", false], ["text-color", false], ["text-size", false], ["border", true], ["font-weight", true]]),
  styleList: new Array(),
  useTagDetail: false,
...
var ColorfulTag = class extends import_obsidian.Plugin {
  ...
  async addListenerForTag() {
    ...
    tags_dom.forEach((tag, i) => {
      ...
      if (__tag_item == void 0)
        return;
      if (new Map(Object.entries(__tag_item)).size == 0)
        return;
  ...
  async addPopup(event, i) {
    ...
    if (_file == void 0) {
      new import_obsidian.Setting(body).setClass("colorful-tag-popup-body-btn").addButton((b) => {
        b.setButtonText("Add");
        b.onClick(async () => {
          let item = /* @__PURE__ */ new Map([["_i", i], ["_l", line], ["_c", col]]);
          for (let [k, v] of tag_item_map.entries()) {
            if (k.charAt(0) != "#") {
              continue;
            }
            let defaultSetting = tag_item_map.get("default-" + k) || "";
            defaultSetting = defaultSetting.replace(/{{DATE}}/g, (0, import_moment.default)().format("YYYY-MM-DD"));
            defaultSetting = defaultSetting.replace(/{{TIME}}/g, (0, import_moment.default)().format("HH:mm:ss"));
            defaultSetting = defaultSetting.replace(/{{DATETIME}}/g, (0, import_moment.default)().format("YYYY-MM-DD HH:mm:ss"));
            defaultSetting = defaultSetting.replace(/{{TAG}}/g, tag);
            item.set(v, defaultSetting);
          }
          tagDetail.set(filename, Object.fromEntries(/* @__PURE__ */ new Map([[`${tag}-${pos}`, Object.fromEntries(item)]])));
      ...
      let _tag = new Map(Object.entries(_file));
  ...
  async refreshPopupBody(body, tag, filename, pos) {
    if (!this.settings.useTagDetail) {
      return;
    }
    let tagDetail = new Map(Object.entries(this.settings.tagDetail));
    let tags = new Map(Object.entries(tagDetail.get(filename)));
    let items = new Map(Object.entries(tags.get(`${tag}-${pos}`)));
    let _tag_item = new Map(Object.entries(this.settings.tagItem));
    let tag_item = new Map(Object.entries(_tag_item.get(tag)));
    for (let [k, v] of tag_item.entries()) {
      if (v.charAt(0) == "_")
        continue;
      if (k.charAt(0) != "#")
        continue;
      let item = new import_obsidian.Setting(body).addText((t) => {
        let item2 = items.get(v);
        t.setValue(item2);
  ...
  async handleFileChange(file, data, cache) {
    if (!this.settings.useTagDetail) {
      return;
    }
    let tags = cache.tags;
    ...
    let _tags = new Map(Object.entries(_file));
    if (_tags.get("tag_num") == tags.length) {
      let _new = /* @__PURE__ */ new Map();
...
var ColorfulTagSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  refreshTagItems(item_setting, tag_name) {
    ...
    let this_tag_item = /* @__PURE__ */ new Map();
    if (_this_tag_item) {
    ...
    for (let [key, value] of this_tag_item.entries()) {
      ...
      new import_obsidian.Setting(item_setting).addText((text) => text.setValue(value).setPlaceholder("key").onChange(async (value2) => {
        this_tag_item.set(key, value2);
        tagItem.set(tag_name, Object.fromEntries(this_tag_item));
        thisSetting.tagItem = Object.fromEntries(tagItem);
        this.plugin.saveSettings();
      })).addDropdown((dropdown) => dropdown.setValue("TEXT").addOption("TEXT", "Text")).addText((text) => text.setValue(defaultSetting).setPlaceholder("default value").onChange(async (value2) => {
        this_tag_item.set("default-" + key, value2);
        tagItem.set(tag_name, Object.fromEntries(this_tag_item));
        thisSetting.tagItem = Object.fromEntries(tagItem);
        this.plugin.saveSettings();
      })).addButton((btn) => btn.setIcon("trash-2").setWarning().onClick(async () => {
    ...
    new import_obsidian.Setting(item_setting).addButton((button) => button.setButtonText("Add item").onClick(async () => {
  ...
  asetting(i, add = false) {
    ...
    div.ondragover = (e) => {
      var _a, _b;
      let target = e.target;
      target = target.matchParent(".colorful-tag-rule-outer");
      if (target.classList.contains("colorful-tag-rule-outer")) {
        if (this.isBefore(this._el, target))
          (_a = target.parentNode) == null ? void 0 : _a.insertBefore(this._el, target);
        else
          (_b = target.parentNode) == null ? void 0 : _b.insertBefore(this._el, target.nextSibling);
    ...
    div.ondragstart = (e) => {
      ...
      if (target.classList.contains("colorful-tag-rule-outer")) {
        this._el = target;
        (_a = e.dataTransfer) == null ? void 0 : _a.setData("effectAllowed", "move");
    ...
    let title = new import_obsidian.Setting(header).addToggle((tg) => tg.setValue(m.get("enable")).onChange((v) => {
      m.set("enable", v);
    ...
    (0, import_obsidian.setIcon)(ctl, "right-triangle");
    ctl.onClickEvent(() => {
    ...
    (0, import_obsidian.setIcon)(ctl_override, "right-triangle");
    ...
    new import_obsidian.Setting(normal).setName("tag").addText((text) => text.setValue(tag_name).onChange(async (value) => {
      m.set("tag", value);
      thisSetting.styleList[i] = Object.fromEntries(m);
    ...
    for (let attr of thisSetting.attrList) {
      ...
      let setting = new import_obsidian.Setting(html).setName(attr).addText((text) => text.setValue(m.get(attr_alt)).onChange(async (value) => {
      ...
      if (global_enable.get(attr_alt)) {
        let override_text = setting.components[0];
    ...
    (0, import_obsidian.setIcon)(ctl_tag_item, "right-triangle");
  ...
  display() {
    ...
    (0, import_obsidian.setIcon)(ctl_general, "right-triangle");
    ...
    new import_obsidian.Setting(general_inner).setName("Enable Tag Detail (beta)").addToggle((toggle) => toggle.setValue(thisSetting.useTagDetail).onChange(async (value) => {
    ...
    (0, import_obsidian.setIcon)(globalCtl, "right-triangle");
    ...
    for (let attr of thisSetting.attrList) {
      let attr_alt = attr.replace(/ /g, "-");
      let setting = new import_obsidian.Setting(globalInner).setName(attr).addText((text) => text.setValue(global.get(attr_alt)).setDisabled(!global_enable.get(attr_alt)).setPlaceholder(thisSetting.defaultStyle.get(attr_alt)).onChange(async (value) => {
        global.set(attr_alt, value);
        this.plugin.settings.global = Object.fromEntries(global);
        this.plugin.saveSettings();
      })).setDesc("leave blank to use default value").addToggle((toggle) => toggle.setValue(global_enable.get(attr_alt) || false).onChange((v) => {
        global_enable.set(attr_alt, v);
        this.plugin.settings.global_enable = Object.fromEntries(global_enable);
        let text = setting.components[0];
        text.setDisabled(!v);
        if (!v) {
          text.setValue("");
    ...
    (0, import_obsidian.setIcon)(ctl_support, "right-triangle");
