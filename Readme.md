...
- 目录结构
	...
	- [更新日志（Changelog）](更新日志（Changelog）.md)：每个版本变更明细；
...
- ✔️插件新增
	- 【CardBoard】（默认开启）
		...
		- 🙁遗憾：
			- 不支持 无序列表（bullet list）内嵌任务，需要无序列表的首个级别就是任务模式（task）；【作者已经受理该诉求，后续会支持更灵活】
...
- 🛠️样式修复
	...
	- 微调：标题（headings) 的默认颜色样式，个人体感不会过于抢眼，若不喜欢也可自行调整。路径：theme settings > minmal > headings； 
